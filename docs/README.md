...
│                       核心层（金库 + 策略）                    │
│  ┌─────────────────────┐      ┌─────────────────────┐         │
│  │  AdaptiveLPVault     │─────▶│  AdaptiveRebalance│         │
│  │  （金库）             │      │  Strategy（策略）  │         │
│  │  - 管理份额          │      │  - 计算波动率       │         │
│  │  - 协调再平衡        │      │  - 资金配比         │         │
│  │  - 双币存取          │      │  - 区间tick计算     │         │
...
│             ▼                                                 │
│  ┌─────────────────────┐      ┌─────────────────────┐         │
│  │  TWAPOracle          │      │  RebalanceIncentives │       │
│  │  （预言机）           │      │  （激励）             │       │
│  │  - 读TWAP价格        │      │  - 记录再平衡        │         │
│  │  - 算时间加权均价     │      │  - 发放USDC奖励      │         │
│  └─────────────────────┘      └─────────────────────┘          │
...
┌────────────────────────────────────────────────────────────────────────────┐
│                       外部协议（Uniswap）                                   │
│  Uniswap V2 Pool         Uniswap V3 Pool (0.05%)   Uniswap V3 Pool (0.30%) │
└────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                       治理层（上层）                             │
│  ┌──────────────────┐  ┌────────────────────┐                   │
│  │  GovernanceToken │  │  AdaptiveGovernance│  管理范围：金库/   │
│  │  （治理代币）     │  │  （治理合约）       │  策略/预言机/激励/ │
│  │  ERC20Votes      │  │  - 提案投票         │  适配器           │
│  │  投票权           │  │  - 时间锁48小时     │                  │
│  └──────────────────┘  │  - executeAsOwner  │                   │
...
> 注：示例基于 Sepolia 测试网当前价格（WETH ≈ 20,000 USDC）

#### 第一步：用户存款

假设用户存入 **1,000 USDC + 0.05 WETH**（双币对等，总价值 2,000 USDC）：
...
假设当前波动率为 **15%（低波动）**，按策略分配：

| 池子 | 分配比例 | 金额（USDC 价值） |
|------|---------|------------------|
| V3 低费率（0.05%） | 30% | 600 |
| V3 高费率（0.30%） | 60% | 1,200 |
| V2 | 10% | 200 |
...
假设 WETH 价格从 20,000 涨到 22,000 USDC（涨幅 10%）：
...
- 按当前净值赎回 WETH + USDC 双币
- 扣除做市手续费收益后的净值计算份额价值
- 获得约 25% 的金库资产（WETH + USDC）
...
| 4 | 无常损失 | 波动率自适应策略 + 三层区间做市 + V2 宽区间摊薄 |
...
5. **不变量验证**：系统核心不变量（如总资产守恒、新存款不稀释现有份额比例）通过模糊测试验证
