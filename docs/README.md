...
**链上提案可修改的 6 项参数**（提案通过后 Governance 自动同步到对应业务合约）：

| 提案类型 | 参数 | 默认值 | 同步到 |
|---------|------|--------|--------|
| SET_TWAP_WINDOW | TWAP 预言机窗口 | 1800 秒（30 分钟） | Oracle |
| SET_REBALANCE_THRESHOLD | 再平衡触发阈值 | 500 bps（5%） | Strategy |
| SET_INCENTIVE_BPS | 再平衡奖励比例 | 500 bps（5%） | Incentives |
| SET_MAX_SLIPPAGE | 最大滑点容忍 | 100 bps（1%） | Vault |
| SET_WEIGHT_CAPS | 三池权重上限 | V2 60% / V3低费 50% / V3高费 70% | Governance 状态 |
| SET_RANGE_BPS | 三档流动性区间宽度 | 紧 ±2% / 中 ±10% / 宽 ±30% | Governance 状态 |

**管理员可直接执行的操作**（不开放提案，由部署者通过 Governance 直接调用）：

| 操作 | 合约 | 说明 |
|------|------|------|
| setAdapters | Vault | 更换 V2/V3 适配器地址 |
| setIncentives | Vault | 更换激励合约地址 |
| setGovernance | Vault / Strategy / Oracle | 转移合约所有权 |
| setPaused | Vault | 紧急暂停（暂停后存取款冻结） |
| setCooldownPeriod | Incentives | 激励冷却期（60~86400 秒） |
| setMinProfitThreshold | Incentives | 触发奖励的最小利润门槛 |
| setVault / setStrategy / setOracle / setIncentives | Governance | 绑定各业务合约地址 |

管理员通过 Governance 的 `executeAsOwner(target, data)` 函数，以上述业务合约 owner 身份调用任意管理函数。

> **部署阶段说明**：当前由部署者通过 Governance 合约直接行使管理权限，系统已预留完整的链上治理升级路径：
>
> - 上线后向社区分发 ALP 治理代币，激活提案与投票功能
> - 将 Governance ownership 转移至时间锁或多签钱包
> - 部署者移交全部管理权限，实现完整链上治理
...
| 奖励比例 | **5%**（治理可调，上限 20%） |
| 最小利润门槛 | **1 USDC** |
| 激励冷却期 | **5 分钟**（治理可调 60~86400 秒；因仅金库可调用，实际受金库 10/30 分钟动态冷却约束，不单独生效） |
| 资金来源 | 手动充值（部署默认 0） |
| 池耗尽时 | 奖励为 0，再平衡照常执行 |

> **注意**：激励池依赖部署后手动充值（直接转账或 `fundRewards`）；未充值或耗尽时，触发者拿不到奖励，但再平衡不受影响。
...
|------|------|---------|
| V2 | 必须双币 | 完全不投 V2 |
| V3 | 单币也可投 | 变成单边仓位，无常损失更大 |
| 完整路由 | 双币 | 三池正常分配 |

> **说明**：**双币存款是推荐路径**。只有同时存入 WETH 和 USDC，金库才能完整参与 V2 + V3 三池配置。如果只存单币（通过标准 ERC4626 接口），V2 完全不参与（V2 要求双币配对），资金只会进入 V3 单边仓位——V3 虽然支持单边存入，但单边仓位在价格波动时无常损失远大于双币仓位。
>
> 因此，**用户必须同时存入 WETH 和 USDC** 才能完整享受金库的多池自动配置能力。前端只暴露双币存款入口，正是为了避免用户误存单币导致路由退化。前端授权为无限授权（MaxUint256），用户需自行管理授权。
...
| 参数 | 默认值 | 治理可调 |
|------|--------|---------|
| 再平衡冷却（常规 / 紧急） | **10 分钟 / 30 分钟** | 否 |
| 波动率分层阈值 | **20% / 50%** | 否 |
| TWAP 窗口 | **30 分钟** | 是（5 分钟 ~ 24 小时） |
| 提案门槛 / 法定人数 | **1000 / 10000 ALP** | 否 |
| 投票延迟 / 投票期 / 时间锁 | **1 块 / 28800 块 / 48 小时** | 否 |
| 激励比例 / 最小利润 / 激励冷却 | **5% / 1 USDC / 5 分钟** | 是 |
| WETH / USDC 灰尘阈值 | **0.000001 / 0.001** | 否 |
| 全局滑点上限 `maxSlippageBps` | **1%** | 是（0.1% ~ 5%） |
| 前端滑点（赎回 / 存款） | **1% / 0（无保护）** | 否（硬编码） |
