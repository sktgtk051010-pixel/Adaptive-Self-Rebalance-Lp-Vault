...
- **为什么选 ERC4626**：兼容主流钱包和 DeFi 生态；前端已封装双币存款/赎回入口，用户直接使用即可
...
┌─────────────────────────────────────────────────────────────────┐
│                          用户交互层                               │
│   存入 WETH+USDC  →  铸造 ALP-VAULT 份额  →  赎回 WETH+USDC     │
└──────────────────────────────┬──────────────────────────────────┘
                               │
┌──────────────────────────────▼──────────────────────────────────┐
│                       核心金库层                                 │
│  ┌──────────────────┐  ┌──────────────────┐  ┌───────────────┐ │
│  │  ERC4626 金库     │  │  再平衡引擎       │  │  再平衡策略    │ │
│  │  (AdaptiveLPVault)│  │  (rebalance)     │  │  (Strategy)   │ │
│  └─────────┬─────────┘  └────────┬─────────┘  └───────┬───────┘ │
│            │                    │                      │         │
│  ┌─────────▼─────────┐  ┌────────▼─────────┐  ┌───────▼───────┐ │
│  │  TWAP 预言机       │  │  再平衡激励       │  │  外部调用者    │ │
│  │  (TWAPOracle)      │  │  (Incentives)   │  │  (任何人触发)  │ │
│  └───────────────────┘  └──────────────────┘  └───────────────┘ │
└──────────────────────────────┬──────────────────────────────────┘
                               │
┌──────────────────────────────▼──────────────────────────────────┐
...
│                       治理层（上层）                              │
│  ┌──────────────────┐  ┌──────────────────┐                    │
│  │  治理代币 ALP-GOV │  │  治理合约         │  管理范围：金库/   │
│  │  (GovernanceToken)│  │  (AdaptiveGov)   │  策略/预言机/激励  │
│  └──────────────────┘  └──────────────────┘                    │
└─────────────────────────────────────────────────────────────────┘
```

> **架构说明**：
> - **治理层在核心金库之上**：治理合约管理金库、策略、预言机、激励四个模块，通过 `executeAsOwner` 以各合约 owner 身份调用管理函数
> - **再平衡激励模块独立**：金库 rebalance 完成后调用激励合约记账，奖励从独立 USDC 资金池发放
> - **任何人可触发再平衡**：无权限限制，冷却期结束后外部调用者即可触发
...
> **领取流程**：再平衡触发时，金库先调用 V3 适配器的 `collectFees()` 领取手续费，再进行新的资金分配；V2 手续费无需主动领取，直接体现在 LP 代币价值中。
...
| 分类 | 操作路径 | 滑点保护 | 说明 |
|------|---------|---------|------|
| **用户操作** | 双币存款 `depositDual` | ⚠️ 前端未设下限 | 用户可传 `minShares`，当前前端传 0 |
| | 双币赎回 `withdrawDual` | ✅ 已启用（1%） | 前端默认 1%，用户可传 `minWETH/minUSDC` |
| **V2 适配器** | 添加/移除流动性 | ✅ 已启用（1%） | adapter 内部自己算滑点下限，外部传参被忽略 |
| **V3 适配器** | rebalance 重新加流动性 | ⚠️ 未启用 | 代码中 `slippageMin` 传参已注释 |
| | rebalance 全量撤出 | ⚠️ 未启用 | 未传 `amount0Min`/`amount1Min` |
| **标准接口** | ERC4626 存/取/赎 | ⚠️ 接口限制 | 标准接口签名不支持滑点参数 |

**已启用**：成交偏离超过容忍度时自动 `revert`；**未启用**：当前版本已知限制，后续迭代恢复传参。
...
| # | 风险 | 缓解措施 |
|---|------|---------|
| 1 | 三明治攻击 | 滑点保护（覆盖主要操作路径）+ 最小输出限制 + TWAP 参考价 |
| 2 | 重入攻击 | ReentrancyGuard + Checks-Effects-Interactions 模式 |
| 3 | 预言机操纵 | TWAP 时间加权均价 + 窗口治理 + 价格偏离校验 |
| 4 | 无常损失 | 波动率自适应策略 + 三层区间做市 + V2 对冲 |
| 5 | 份额价格操纵（首存/捐赠攻击） | 双币存款天然抗捐赠 + 灰尘阈值保护 + 首存 1:1 计价 |
| 6 | ERC4626 第三方兼容性警告 | 前端仅暴露双币入口 + 标准接口仅 USDC 路径 |
...
2. **反向路径覆盖**：核心 require/revert 错误路径均有对应测试；分支覆盖率 60.44%，受 viaIR 优化下 forge coverage 分支统计精度限制，实际测试覆盖范围高于报告数值
